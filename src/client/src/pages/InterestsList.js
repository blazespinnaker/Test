...
class InterestsList extends Component {
  ...
  confirmDialogDelete(id) {
    this.props.actionsInterests.deleteInterests(this.state.idDelete).then(data => {
      this.props.actionsInterests.loadInterestsList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
            ...
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/interestses/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.InterestsListReducer.listInterests
  };
}
