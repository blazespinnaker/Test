...
class QualitiesList extends Component {
  ...
  confirmDialogDelete(id) {
    this.props.actionsQualities.deleteQualities(this.state.idDelete).then(data => {
      this.props.actionsQualities.loadQualitiesList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
  ...
  render() {
    ...
    const link = "/qualitieses/";

    return (
      <div>
        <h1>Qualities List</h1>
            ...
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/qualitieses/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.QualitiesListReducer.listQualities
  };
}
