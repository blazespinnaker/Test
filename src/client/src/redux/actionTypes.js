/* 
* Generated by
* 
*      _____ _          __  __      _     _
*     / ____| |        / _|/ _|    | |   | |
*    | (___ | | ____ _| |_| |_ ___ | | __| | ___ _ __
*     \___ \| |/ / _` |  _|  _/ _ \| |/ _` |/ _ \ '__|
*     ____) |   < (_| | | | || (_) | | (_| |  __/ |
*    |_____/|_|\_\__,_|_| |_| \___/|_|\__,_|\___|_|
*
* The code generator that works in many programming languages
*
*			https://www.skaffolder.com
*
*
* You can generate the code from the command-line
*       https://npmjs.com/package/skaffolder-cli
*
*       npm install -g skaffodler-cli
*
*   *   *   *   *   *   *   *   *   *   *   *   *   *   *   *
*
* To remove this comment please upgrade your plan here: 
*      https://app.skaffolder.com/#!/upgrade
*
* Or get up to 70% discount sharing your unique link:
*       https://app.skaffolder.com/#!/register?friend=5d44e33ca046842f16aa4050
*
* You will get 10% discount for each one of your friends
* 
*/

// INSERT HERE YOUR CUSTOM CONSTANTS




// START CRUD CONSTANTS

// INTERESTS
export const CREATE_INTERESTS_SUCCESS = "CREATE_INTERESTS_SUCCESS";
export const DELETE_INTERESTS_SUCCESS = "DELETE_INTERESTS_SUCCESS";
export const GET_INTERESTS_SUCCESS = "GET_INTERESTS_SUCCESS";
export const LIST_INTERESTS_SUCCESS = "LIST_INTERESTS_SUCCESS";
export const UPDATE_INTERESTS_SUCCESS = "UPDATE_INTERESTS_SUCCESS";

// QUALITIES
export const CREATE_QUALITIES_SUCCESS = "CREATE_QUALITIES_SUCCESS";
export const DELETE_QUALITIES_SUCCESS = "DELETE_QUALITIES_SUCCESS";
export const GET_QUALITIES_SUCCESS = "GET_QUALITIES_SUCCESS";
export const LIST_QUALITIES_SUCCESS = "LIST_QUALITIES_SUCCESS";
export const UPDATE_QUALITIES_SUCCESS = "UPDATE_QUALITIES_SUCCESS";

// SCORES
export const CREATE_SCORES_SUCCESS = "CREATE_SCORES_SUCCESS";
export const DELETE_SCORES_SUCCESS = "DELETE_SCORES_SUCCESS";
export const FINDBYINTEREST_SCORES_SUCCESS = "FINDBYINTEREST_SCORES_SUCCESS";
export const FINDBYQUALITY_SCORES_SUCCESS = "FINDBYQUALITY_SCORES_SUCCESS";
export const GET_SCORES_SUCCESS = "GET_SCORES_SUCCESS";
export const LIST_SCORES_SUCCESS = "LIST_SCORES_SUCCESS";
export const UPDATE_SCORES_SUCCESS = "UPDATE_SCORES_SUCCESS";

// USER
export const CHANGEPASSWORD_USER_SUCCESS = "CHANGEPASSWORD_USER_SUCCESS";
export const CREATE_USER_SUCCESS = "CREATE_USER_SUCCESS";
export const DELETE_USER_SUCCESS = "DELETE_USER_SUCCESS";
export const GET_USER_SUCCESS = "GET_USER_SUCCESS";
export const LIST_USER_SUCCESS = "LIST_USER_SUCCESS";
export const UPDATE_USER_SUCCESS = "UPDATE_USER_SUCCESS";

// END CRUD CONSTANTS

// USER
export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGOUT_SUCCESS = "LOGOUT_SUCCESS";
export const CHANGE_PASSWORD_SUCCESS = "CHANGE_PASSWORD_SUCCESS";
export const CHANGE_PASSWORD_ADMIN_SUCCESS = "CHANGE_PASSWORD_ADMIN_SUCCESS";
